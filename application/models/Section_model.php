...
class Section_model extends CI_Model 
{
        ...
        try
        {
            ...
            if ($query)
            {
                ...
                return array(
                    'status' => 1, 
                    'data' => $result, 
                    'total' => $this->db->count_all_results('section'),
                );
            }
    ...
    public function insert($posts) 
    {
        try
        {
            ...
            self::$check_code = $this->db->query("SELECT EXISTS(SELECT `code` FROM section WHERE `code` = '$code') AS `exists`")->row();

            self::$check_name = $this->db->query("SELECT EXISTS(SELECT `name` FROM section WHERE `name` = '$name') AS `exists`")->row();
            ...
            if ($validation_errors == null)
            {
                ...
                $insert_query = $this->db->set($posts)->insert('section');
    
                if ($insert_query)
                {
                    $message = 'Section has been added.';

                    $this->audit_model->log('Section', 'Add', $message, $by_user_id);
            ...
            {
                return array('status' => 0, 'message' => 'Add section failed.', 'validation_errors' => $validation_errors);
            }
    ...
    public function update($posts)
    {
        try
        {
            ...
            $row = $this->db->query("SELECT code, name from section WHERE id = '$id'")->row();
            ...
            if ($validation_errors == null)
            {
                ...
                $update_query = $this->db->where('id', $id)->update('section', $posts);

                if ($update_query)
                {
                    $message = "Section has been updated.";

                    $this->audit_model->log('Section', 'Update', $message, $by_user_id);
            ...
            {
                return array('status' => 0, 'message' => 'Update section failed.', 'validation_errors' => $validation_errors);
            }
        }
        catch (Exception $e)
        {
    ...
    public function delete($posts)
    {
        try
        {
            if (!isset($posts['ids']))
            {
                return array('status' => 0, 'message' => 'Delete section/s failed.');
            }

            $delete_query = $this->db->where_in('id', json_decode($posts['ids']))->delete('section');

            if ($delete_query)
            {
                $this->audit_model->log('Section', 'Delete', 'Section/s has been deleted.', $posts['by_user_id']);

                return array('status' => 1, 'message' => "Section/s has been deleted.");
            }
